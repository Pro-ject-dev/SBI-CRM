...
export function generatePurchaseOrderPdf(
  ...
  opts?: {
    ...
    deliveryBy?: string;
    taxes?: { cgstPct?: number; sgstPct?: number; igstPct?: number; roundOff?: number };
    notes?: {
      taxes?: string;
      payment?: string;
      delivery?: string;
  ...
  try {
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        1: { cellWidth: 40 },                     // Item Description - increased
        2: { cellWidth: 33 },                     // Spec/Make - increased
        3: { cellWidth: 9, halign: "center" },   // UOM
        4: { cellWidth: 9, halign: "center" },   // Qty
        5: { cellWidth: 15, halign: "center" },  // Delivery
        6: { cellWidth: 22, halign: "right" },   // Unit Price
        7: { cellWidth: 28, halign: "right" },   // Total Value
      },
    ...
    const cgstPct = opts?.taxes?.cgstPct ?? Number((order as any)?.cgst || 0);
    const sgstPct = opts?.taxes?.sgstPct ?? Number((order as any)?.sgst || 0);
    const igstPct = opts?.taxes?.igstPct ?? Number((order as any)?.igst || 0);

    const cgstVal = basicValue * (cgstPct / 100);
    const sgstVal = basicValue * (sgstPct / 100);
    const igstVal = basicValue * (igstPct / 100);

    const roundOff = opts?.taxes?.roundOff ?? 0;
    const netTotal = basicValue + cgstVal + sgstVal + igstVal + roundOff;
    ...
    if (igstPct > 0) {
      totalBody.push([`IGST ${igstPct}%`, formatCurrency(igstVal).replace('₹', '')]);
    } else {
      if (cgstPct > 0) totalBody.push([`CGST ${cgstPct}%`, formatCurrency(cgstVal).replace('₹', '')]);
      if (sgstPct > 0) totalBody.push([`SGST ${sgstPct}%`, formatCurrency(sgstVal).replace('₹', '')]);
    }

    if (roundOff) {
      totalBody.push([`Round off ${roundOff >= 0 ? '' : '(-)'}`, formatCurrency(Math.abs(roundOff)).replace('₹', '')]);
    }

    totalBody.push(["Net Total", formatCurrency(netTotal).replace('₹', '')]);
