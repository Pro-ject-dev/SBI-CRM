      ...
      <Paper elevation={1} sx={{ p: 3, borderRadius: "16px", border: "1px solid #e0e0e0" }}>
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} sm={6}>
            ...
            <InputBox id="grade" name="grade" type="text" value={grade} onChange={(_, val) => setGrade(val)} />
          </Grid>
          <Grid item xs={12}>
            <Typography variant="caption" display="block" gutterBottom sx={{ fontWeight: 500, color: "text.secondary" }}>Remark</Typography>
            <InputBox id="remark" name="remark" type="text" value={remark} onChange={(_, val) => setRemark(val)} />
          </Grid>
        </Grid>
