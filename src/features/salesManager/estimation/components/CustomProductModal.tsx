...
const CustomProductModal: React.FC<CustomProductModalFormProps> = ({
  ...
  useEffect(() => {
    if (open) {
      ...
      if (comboList.length === 0) {
        ...
        fetch("https://sbiapi.sustainnovatechlabs.com/api/admin/getCombos", {
  ...
  useEffect(() => {
    if (selectedIds.comboId && open) {
      ...
      fetch(`https://sbiapi.sustainnovatechlabs.com/api/admin/getCategorybyCombo?comboId=${selectedIds.comboId}`, {
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      })
        .then((res) => res.json())
        .then((data) => {
  ...
  useEffect(() => {
    if (selectedIds.comboId && open) {
      ...
      fetch("https://sbiapi.sustainnovatechlabs.com/api/admin/getProductbyCombo&Category?isStandard=0", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
