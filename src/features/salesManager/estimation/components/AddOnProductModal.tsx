...
const AddOnProductModal: React.FC<ProductFormModalProps> = ({
  ...
  const fetchAvailableProducts = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch('https://sbiapi.sustainnovatechlabs.com/api/admin/getAddons', { headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` } });
  ...
  const handleProductSelectChange = async (event: SelectChangeEvent<string>) => {
    ...
    if (productId) {
      ...
      try {
        const response = await fetch(`https://sbiapi.sustainnovatechlabs.com/api/admin/getAddonsbyId?id=${productId}`, { headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` } });
