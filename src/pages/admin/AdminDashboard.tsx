...
const AdminDashboard = () => {
  ...
  const { data: purchaseOrdersData, isLoading: poLoading } =
    useGetPurchaseOrdersQuery();
  const { data: rawMaterialsData, isLoading: rmLoading } =
  ...
  const { data: consumableStatsData, isLoading: consumableStatsLoading } =
    useGetConsumableMaterialsStatsQuery();
  const { data: consumableAlertsData, isLoading: consumableAlertsLoading } =
    useGetConsumableStockAlertsQuery({});

  // Handle different API response structures with proper fallbacks
  const leadsArray = Array.isArray(leadsData)
    ? leadsData
    : (leadsData as any)?.data && Array.isArray((leadsData as any).data)
      ? (leadsData as any).data
      : [];
  const ordersArray = Array.isArray(ordersData)
    ? ordersData
    : (ordersData as any)?.data && Array.isArray((ordersData as any).data)
  ...
  const consumableStats = consumableStatsData?.data;
  const consumableAlerts =
    consumableAlertsData?.data?.filter((alert) => !alert.isRead) || [];
    ...
    <Dashboard title="Admin Dashboard">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {kpiData.map((kpi, index) => (
          <div
            key={index}
            className="group bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-all duration-200 hover:border-gray-200"
          >
            <div className="flex items-start justify-between">
              ...
              <div
                className={`bg-gradient-to-r ${kpi.color} p-3 rounded-xl shadow-sm`}
              >
                {kpi.icon}
              </div>
            </div>
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="font-semibold text-gray-900">
                          ₹{" "}
                          {Number(
                            order.estimation?.grandTotal || 0,
                          ).toLocaleString()}
                        </p>
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${statusColors[order.orderStatus] || "bg-gray-100 text-gray-800 border-gray-200"}`}
                        >
      ...
      {consumableAlerts.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {consumableAlerts.slice(0, 6).map((alert: any) => (
                <div
                  key={alert.id}
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2 text-sm">
                        <span className="text-orange-600 font-semibold">
                          ...
                          {alert.consumableMaterial?.unit || "units"}
                        </span>
                        <span className="text-gray-400">|</span>
                        <span className="text-gray-600">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100 bg-gray-50">
          ...
          <p className="text-sm text-gray-600 mt-1">
            Frequently used functions for faster access
          </p>
        </div>
        <div className="p-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              {
                to: "/admin/purchase-orders-approval",
                title: "Manage POs",
                icon: BarChart2,
                color: "from-orange-500 to-orange-600",
